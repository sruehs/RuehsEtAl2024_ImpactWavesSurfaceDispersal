...
# - Filename: CalcSpeed_fulldepth_temporalmeans.py
...
# Calculates long-term and seasonal averages of gridded Lagrangian speed
...
uvars_ugrid_nc = xr.open_mfdataset(list_ugrid_nc,
                                   combine='by_coords')
vvars_vgrid_nc = xr.open_mfdataset(list_vgrid_nc,
                                   combine='by_coords')
# on lorenz, uncoupled experiment contains data from 2018 to 2020,
# coupled experiment contains data only from 2019 to 2020
# -> need to restrict uncoupled one to period of coupled one
#uvars_ugrid_nc = xr.open_mfdataset(list_ugrid_nc[365:],
#                                   combine='by_coords')
#vvars_vgrid_nc = xr.open_mfdataset(list_vgrid_nc[365:],
#                                   combine='by_coords')
uvars_ugrid_c = xr.open_mfdataset(list_ugrid_c,
                                  combine='by_coords')
vvars_vgrid_c = xr.open_mfdataset(list_vgrid_c,
                                  combine='by_coords')
...
for i in range(0, 1, 1):
    ...
    # 2.1 Rename depth coordinate/dimension
    # (it is the same for both variables)
    uvars_ugrid_depth_nc = uvars_ugrid_nc.isel(
        depthu=i).rename({'depthu': 'z'})
    vvars_vgrid_depth_nc = vvars_vgrid_nc.isel(
        depthv=i).rename({'depthv': 'z'})
    uvars_ugrid_depth_c = uvars_ugrid_c.isel(
        depthu=i).rename({'depthu': 'z'})
    vvars_vgrid_depth_c = vvars_vgrid_c.isel(
        depthv=i).rename({'depthv': 'z'})

    # 2.2 Interpolate
    vvars_c = vvars_vgrid_depth_c.interp_like(
        uvars_ugrid_depth_c, method='linear')
    vvars_nc = vvars_vgrid_depth_nc.interp_like(
        uvars_ugrid_depth_nc, method='linear')
    ...
    speed_Enc_tsave = speed_Enc.groupby(
        'time_counter.season').mean(dim='time_counter')
    speed_Ec_tsave = speed_Ec.groupby(
        'time_counter.season').mean(dim='time_counter')
    speed_Sc_tsave = speed_Sc.groupby(
        'time_counter.season').mean(dim='time_counter')
    speed_EcSc_tsave = speed_EcSc.groupby(
        'time_counter.season').mean(dim='time_counter')
    speed_EncSc_tsave = speed_EncSc.groupby(
        'time_counter.season').mean(dim='time_counter')
    speed_ScProjEcSc_tsave = speed_ScProjEcSc.groupby(
        'time_counter.season').mean(dim='time_counter')
    speed_EcProjEcSc_tsave = speed_EcProjEcSc.groupby(
        'time_counter.season').mean(dim='time_counter')
    ...
    if zlev < 10:
        speed_tave_ds.to_netcdf(
            f'{outpath_data}Speed-tave-fulldepth-z00{zlev}.nc')
    if (zlev >= 10) and (zlev < 100):
        speed_tave_ds.to_netcdf(
            f'{outpath_data}Speed-tave-fulldepth-z0{zlev}.nc')
    if (zlev >= 100):
        speed_tave_ds.to_netcdf(
            f'{outpath_data}Speed-tave-fulldepth-z{zlev}.nc')
    ...
    speed_tsave_ds = xr.Dataset(data_vars={'Enc': speed_Enc_tsave.where(speed_Enc_tsave != 0),
                                ...
                                coords={'nav_lon': (('y', 'x'), nav_lon),
                                        'nav_lat': (('y', 'x'), nav_lat)})
    if zlev < 10:
        speed_tsave_ds.to_netcdf(
            f'{outpath_data}Speed-tseasonave-fulldepth-z00{zlev}.nc')
    if (zlev >= 10) and (zlev < 100):
        speed_tsave_ds.to_netcdf(
            f'{outpath_data}Speed-tseasonave-fulldepth-z0{zlev}.nc')
    if (zlev >= 100):
        speed_tsave_ds.to_netcdf(
            f'{outpath_data}Speed-tseasonave-fulldepth-z{zlev}.nc')
